...
+(void)SaveDictionary:(NSDictionary *)dict andFileName:(NSString *)dictionaryName{
    ...
    BOOL isYES = [mDict writeToFile:path atomically:YES];
    
    NSLog(@"保存----%@----%@",(isYES)?@"成功" : @"失败",dictionaryName);
}
/**
 *从沙盒中获取字典
 *@arrayName
...
+(void)SaveModel:(id)model andFileName:(NSString *)modelName{
    ...
    //将自定义的对象保存到文件中
    BOOL isYES = [NSKeyedArchiver archiveRootObject:model toFile:path];
    
    NSString* msg = isYES? @"成功" : @"失败";
    NSLog(@"保存-%@-%@\n",modelName,msg);
}
